...
## docker cmd example
look up the latest version tag (e.g. v0.0.6)
```
docker run -it --rm -p 8080:80 -p 8443:443 guacozy/guacozy-server:v0.0.6
```
...
Start by looking up the latest version tag (e.g. v0.0.6)
```
# docker-compose.yml
...
services:
  server:
    image: guacozy/guacozy-server:0.0.6
    restart: always
    depends_on:
      - db
    volumes:
